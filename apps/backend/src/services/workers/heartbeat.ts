...
export class HeartbeatMonitor {
  ...
  start() {
    this.interval = setInterval(async () => {
      ...
      // Optimization: Check worker heartbeats and persist state concurrently with Promise.all
      // reduces total heartbeat cycle latency from O(N) sequential executions to O(1) concurrent latency.
      await Promise.all(
        workers.map(async (w) => {
          try {
