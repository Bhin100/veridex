...
export async function evaluateOpportunity(opportunity: any, opts: { policyId?: string } = {}): Promise<EngineResult> {
  ...
  // 2) Evaluate factors concurrently while maintaining deterministic alphabetical output order
  // Optimization: Evaluate all factor functions in parallel via Promise.all rather than awaiting each factor sequentially (O(N) latency -> O(1) concurrent latency)
  ...
  const factorEvaluations = await Promise.all(
    factorNames.map(async (name) => {
      try {
        ...
        return { name, result: { raw: 0, normalized: 0 } }
      } catch (err) {
        return { name, result: { raw: 0, normalized: 0, details: { error: String(err) } } }
      }
    })
  )

  const factorResults: Record<string, FactorResult> = {}
  for (const { name, result } of factorEvaluations) {
    factorResults[name] = result
  }
